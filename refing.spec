...
mkdir -p $RPM_BUILD_ROOT/usr/share/refing-%{version}/refing/

# Copy the rEFIng binaries (rEFIng proper and drivers) to /usr/share/refing-%{version},
...
if [ -f %{keydir}/refind.key ] && [ -x $SBSign ] ; then
   $SBSign --key %{keydir}/refind.key --cert %{keydir}/refind.crt --output $RPM_BUILD_ROOT/usr/share/refing-%{version}/refing/refing_%{efiarch}.efi refing/refing_%{efiarch}.efi
   mkdir -p $RPM_BUILD_ROOT/usr/share/refing-%{version}/refing/drivers_%{efiarch}
   for File in `ls drivers_%{efiarch}/*_x64.efi` ; do
      $SBSign --key %{keydir}/refind.key --cert %{keydir}/refind.crt --output $RPM_BUILD_ROOT/usr/share/refing-%{version}/refing/$File $File
   done
   mkdir -p $RPM_BUILD_ROOT/usr/share/refing-%{version}/refing/tools_%{efiarch}
   $SBSign --key %{keydir}/refind.key --cert %{keydir}/refind.crt --output $RPM_BUILD_ROOT/usr/share/refing-%{version}/refing/tools_%{efiarch}/gptsync_%{efiarch}.efi gptsync/gptsync_%{efiarch}.efi
else
   install -Dp -m0644 refing/refing*.efi $RPM_BUILD_ROOT/usr/share/refing-%{version}/refing/
   mkdir -p $RPM_BUILD_ROOT/usr/share/refing-%{version}/refing/drivers_%{efiarch}
   cp -a drivers_%{efiarch}/* $RPM_BUILD_ROOT/usr/share/refing-%{version}/refing/drivers_%{efiarch}/
   mkdir -p $RPM_BUILD_ROOT/usr/share/refing-%{version}/refing/tools_%{efiarch}
   install -Dp -m0644 gptsync/gptsync_%{efiarch}.efi $RPM_BUILD_ROOT/usr/share/refing-%{version}/refing/tools_%{efiarch}/gptsync_%{efiarch}.efi
fi

# Copy configuration and support files to /usr/share/refing-%{version}
install -Dp -m0644 refing.conf-sample $RPM_BUILD_ROOT/usr/share/refing-%{version}/refing/
cp -a icons $RPM_BUILD_ROOT/usr/share/refing-%{version}/refing/
rm -rf $RPM_BUILD_ROOT/usr/share/refing-%{version}/refing/icons/svg
install -Dp -m0755 refing-install $RPM_BUILD_ROOT/usr/share/refing-%{version}/

# Copy documentation to /usr/share/doc/refing-%{version}
mkdir -p $RPM_BUILD_ROOT/usr/share/doc/refing-%{version}
cp -a docs/Styles $RPM_BUILD_ROOT/usr/share/doc/refing-%{version}/
cp -a docs/refind $RPM_BUILD_ROOT/usr/share/doc/refing-%{version}/
install -Dp -m0644 NEWS.txt COPYING.txt LICENSE.txt README.txt CREDITS.txt $RPM_BUILD_ROOT/usr/share/doc/refing-%{version}

# Copy man pages to /usr/share/man/man8
mkdir -p $RPM_BUILD_ROOT/usr/share/man/man8
install -Dp -m0644 docs/man/mvrefing.8 $RPM_BUILD_ROOT/usr/share/man/man8
install -Dp -m0644 docs/man/mkrlconf.8 $RPM_BUILD_ROOT/usr/share/man/man8
install -Dp -m0644 docs/man/refing-install.8 $RPM_BUILD_ROOT/usr/share/man/man8
install -Dp -m0644 docs/man/refing-mkdefault.8 $RPM_BUILD_ROOT/usr/share/man/man8
install -Dp -m0644 docs/man/refing-sb-healthcheck.8 $RPM_BUILD_ROOT/usr/share/man/man8

# Copy keys to /etc/refing.d/keys
mkdir -p $RPM_BUILD_ROOT/etc/refing.d/keys
install -Dp -m0644 keys/* $RPM_BUILD_ROOT/etc/refing.d/keys

# Copy scripts to /usr/sbin
mkdir -p $RPM_BUILD_ROOT/usr/sbin
install -Dp -m0755 mkrlconf $RPM_BUILD_ROOT/usr/sbin/
install -Dp -m0755 mvrefing $RPM_BUILD_ROOT/usr/sbin/
install -Dp -m0755 refing-mkdefault $RPM_BUILD_ROOT/usr/sbin/
install -Dp -m0755 refing-sb-healthcheck $RPM_BUILD_ROOT/usr/sbin/
ln -sr $RPM_BUILD_ROOT/usr/share/refing-%{version}/refing-install $RPM_BUILD_ROOT/usr/sbin

# Copy banners and fonts to /usr/share/refing-%{version}
cp -a banners $RPM_BUILD_ROOT/usr/share/refing-%{version}/
cp -a fonts $RPM_BUILD_ROOT/usr/share/refing-%{version}/
...
%files
%defattr(-,root,root -)
%doc /usr/share/doc/refing-%{version}
%doc /usr/share/man/man8
/usr/sbin/mkrlconf
/usr/sbin/mvrefing
/usr/sbin/refing-install
/usr/sbin/refing-mkdefault
/usr/sbin/refing-sb-healthcheck
/usr/share/refing-%{version}
/etc/refing.d/
...
# Run the rEFIng installation script. Do so with the --shim option if
...
if [[ $IsSecureBoot == "1" && -n $ShimFile && -n "$BootShimFile" ]] ; then
   if [[ -n $SBSign && -n $OpenSSL ]] ; then
      ./refing-install --shim $ShimFile --localkeys --yes
   else
      ./refing-install --shim $ShimFile --yes
   fi
else
   if [[ -n $SBSign && -n $OpenSSL ]] ; then
      ./refing-install --localkeys --yes
   else
      ./refing-install --yes
   fi
fi
...
# installed by refing-install, since that script will run after an update,
