...
static EFI_STATUS CopyFiles(IN EFI_FILE_PROTOCOL *TargetDir) {
    ...
    CHAR16          *SourceDriversDir, *TargetDriversDir, *RefingName;
    EFI_STATUS      Status;
    EFI_STATUS      WorstStatus = EFI_SUCCESS;
    ...
    RefingName = PoolPrint(L"EFI\\refing\\%s", INST_REFING_NAME);
    Status = CopyOneFile(SourceVolume->RootDir, SourceFile, TargetDir, RefingName);
    MyFreePool(SourceFile);
    MyFreePool(RefingName);
    if (EFI_ERROR(Status)) {
    ...
    if (Status == EFI_SUCCESS) {
        ...
        //  - Copy refing.conf-sample, not refing.conf, if it's available, to
        //    avoid picking up live-disk-specific customizations.
        //  - Do not overwrite an existing refing.conf at the target; instead,
        //    copy to refing.conf-sample if refing.conf is present.
        ConfFile = PoolPrint(L"%s\\refing.conf-sample", SourceDir);
        if (FileExists(SourceVolume->RootDir, ConfFile)) {
            ...
            SourceFile = PoolPrint(L"%s\\refing.conf", SourceDir);
        }
        MyFreePool(ConfFile);
        // Note: CopyOneFile() logs errors if they occur
        if (FileExists(TargetDir, L"\\EFI\\refing\\refing.conf")) {
            Status = CopyOneFile(SourceVolume->RootDir, SourceFile, TargetDir, L"EFI\\refing\\refing.conf-sample");
        } else {
            Status = CopyOneFile(SourceVolume->RootDir, SourceFile, TargetDir, L"EFI\\refing\\refing.conf");
        }
        if (EFI_ERROR(Status))
            WorstStatus = Status;
        MyFreePool(SourceFile);
        ...
        Status = CopyDirectory(SourceVolume->RootDir, SourceFile, TargetDir, L"EFI\\refing\\icons");
        if (EFI_ERROR(Status)) {
        ...
        TargetDriversDir = PoolPrint(L"EFI\\refing\\%s", INST_DRIVERS_SUBDIR);
...
static VOID CreateFallbackCSV(IN EFI_FILE_PROTOCOL *TargetDir) {
    ...
    Status = refit_call5_wrapper(TargetDir->Open, TargetDir, &FilePtr, L"\\EFI\\refing\\BOOT.CSV",
                                 EFI_FILE_MODE_READ | EFI_FILE_MODE_WRITE | EFI_FILE_MODE_CREATE, 0);
    if (Status == EFI_SUCCESS) {
...
static BOOLEAN CopyRefingFiles(IN EFI_FILE_PROTOCOL *TargetDir) {
    EFI_STATUS Status = EFI_SUCCESS, Status2;

    if (FileExists(TargetDir, L"\\EFI\\refing\\icons")) {
        Status = BackupOldFile(TargetDir, L"\\EFI\\refing\\icons");
        if (EFI_ERROR(Status)) {
...
static EFI_STATUS CreateNvramEntry(EFI_HANDLE DeviceHandle) {
    ...
    ProgName = PoolPrint(L"\\EFI\\refing\\%s", INST_REFING_NAME);
...
VOID InstallRefing(VOID) {
    ...
    if (SelectedESP) {
        Status = CopyRefingFiles(SelectedESP->RootDir);
        if (Status == EFI_SUCCESS)
